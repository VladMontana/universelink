    ...
    <template v-if="isAuthenticated">
      ...
      <main class="main">
        <div class="logo-wrapper">
          ...
          <p class="tagline">Сервис по сокращению ссылок с самоуничтожением и аналитикой</p>
        </div>
        ...
        <transition name="slide-up">
          <div v-if="selectedLink" class="link-detail">
            <div class="detail-header">
              ...
              <button class="close-btn" @click="selectedLink = null">✕</button>
            </div>
...
.sidebar-item.active { background: rgba(124,58,237,0.15); border-color: rgba(124,58,237,0.25); }

.sidebar-item-top { display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px; }
.sidebar-short { font-family: var(--mono); font-size: 12px; color: var(--purple2); }
.sidebar-date { font-size: 11px; color: var(--muted); }
.sidebar-item-bottom { display: flex; justify-content: space-between; align-items: center; gap: 8px; }
.sidebar-original { font-size: 11px; color: var(--muted); flex: 1; min-width: 0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.sidebar-badges { display: flex; gap: 4px; flex-shrink: 0; }

.badge { font-size: 10px; font-family: var(--mono); padding: 1px 6px; border-radius: 4px; }
.badge-click { background: rgba(124,58,237,0.2); color: var(--purple2); }
.badge-time  { background: rgba(234,179,8,0.15); color: #fbbf24; }
...
.tagline { font-size: 13px; color: var(--muted); letter-spacing: 0.05em; font-family: var(--mono); }

.input-wrapper { position: relative; width: 100%; max-width: 620px; }
...
.detail-short { font-family: var(--mono); font-size: 14px; color: var(--purple2); flex: 1; }
...
.close-btn { background: none; border: none; color: var(--muted); cursor: pointer; font-size: 14px; padding: 4px; transition: color 0.15s; }
.close-btn:hover { color: var(--text); }

.detail-row { display: flex; gap: 12px; margin-bottom: 16px; align-items: baseline; }
.detail-label { font-size: 11px; text-transform: uppercase; letter-spacing: 0.1em; color: var(--muted); flex-shrink: 0; font-family: var(--mono); }
.detail-url { font-size: 12px; color: var(--muted); text-decoration: none; word-break: break-all; line-height: 1.5; transition: color 0.15s; }
...
.modal-field { display: flex; flex-direction: column; gap: 8px; flex: 1; }
...
.optional { text-transform: none; letter-spacing: 0; font-size: 10px; color: var(--muted); }
